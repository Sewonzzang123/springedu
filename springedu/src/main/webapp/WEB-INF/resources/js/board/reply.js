"use strict";
//후에 session에서 가져옴
const g_bnum = '493'; //게시원글 (임의)
const g_rid = 'test1@test.com'; //작성자(임의)
const g_rnickname = '테스터1'; //작성자 별명
...
function modifyComment(e){
   ...
   // 4 : 서버가 클라이언트의 요청을 완료했고 서버도 응답이 완료된상태
	...
	//3) 요청 메시지
	const rnum =  e.target.closest('.comment').getAttribute('data-rnum');
	const rcontent = e.target.closest('.comment').querySelector('.rcontent').innerHTML;
	const sendData = {};
...
//	sendData.rnickname = g_rnickname;
	...
	//4) javascript 객체 => json문자열 포맷으로 변환
...
function replyComment(e){
   ...
   // 4 : 서버가 클라이언트의 요청을 완료했고 서버도 응답이 완료된상태
	...
	const rcontent = e.target.closest('.comment').querySelector('.rcontent').innerHTML;
	const prnum= e.target.closest('.comment').previousElementSibling.getAttribute('data-rnum');
	...
	list.forEach(comment=>{
	 ...
	 //댓글
    	...
    	str += `<div class="child" data-rnum="${comment.rnum}" data-prnum="${comment.prnum}">`;
			...
			str += `        <div class="nickname">${comment.rnickname}</div>`;
