...
comment
  ...
  .addEventListener(
    "focus",
    (e) => (e.target.nextElementSibling.style.display = "block")
  );
...
comments.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("btn-rereply")) {
    ...
    //클래스명 변경
    comment_clone
      .querySelector(".btn-write")
      .classList.replace("btn-write", "btn-replyWrite");
  ...
  //수정
    ...
    //복제한 노드의 댓글내용 채우기
    comment_clone.querySelector(
      ".rcontent"
    ).innerHTML = parentComment.querySelector(".rcontent").innerHTML;
    ...
    comment_clone
      .querySelector(".btn-write")
      .classList.replace("btn-write", "btn-modifyWrite");
  ...
  //숨김아이템
    ...
    //댓글목록의 숨김아이템을 모두 숨김
    Array.from(comments.querySelectorAll(".btn-ellipsis")).forEach(
      (element) => {
...
(function modal() {
  ...
  const modalDeleteBtn = document.querySelector(".modal__delete");
