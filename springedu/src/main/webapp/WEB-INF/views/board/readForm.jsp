					...
					<ul>
						<li><form:label path="boardCategoryVO.cid">분류</form:label> <form:select
								...
								<form:options path="boardCategoryVO.cid"
									items="${boardCategory }" itemValue="cid" itemLabel="cname" />
							</form:select> <span class="client_msg" id="boardCategoryVO.cid.error"></sapn>
						...
						<li class="umode"><form:label path="files">첨부</form:label> <form:input
								type="file" path="files" multiple="multiple" /> <span
							class="client_msg" id="files.error"></sapn> <form:errors
									cssClass="svr_msg" path="files" /></li>
						<li class="btnGrp">
								...
								<div id="fileList">
									<c:forEach var="file" items="${requestScope.files }">
										<p style="display: flex">
											...
											<span>(${file.fsize/1000 } kb)</span> <span class="umode">
												<a href="#"><i style="padding-left: 5px"
													class="fas fa-backspace" data-fid="${file.fid }"></i></a>
											</span>
										</p>
									</c:forEach>
								</div>
